      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid md:grid-cols-2 gap-6 md:gap-8">
          {benefits.map(({ title, description, iconPath }, idx) => (
            <div
              key={title}
              className="flex gap-4 md:gap-5 items-start bg-white border border-neutral-200 rounded-2xl p-5 md:p-6 shadow-[0_18px_45px_rgba(0,0,0,0.06)]"
              data-animate-on-scroll={idx % 2 === 0 ? "animate-swipe-right-strong" : "animate-swipe-left-strong"}
              data-animate-delay={`${idx * 0.1 + 0.1}s`}
            >
              <div className="w-12 h-12 md:w-14 md:h-14 rounded-full bg-white border border-neutral-200 flex items-center justify-center shrink-0 shadow-[0_10px_20px_rgba(0,0,0,0.05)]">
