const FindSection = () => {
  ...
  return (
    <section className="py-16 md:py-24 px-6 lg:px-16 bg-background relative overflow-hidden">
      ...
      <h2 className="relative font-display text-3xl md:text-4xl lg:text-5xl text-center mb-12 md:mb-16 animate-rise-bloom">
        3 күнде <span className="text-primary">табасың:</span>
      </h2>

      <div className="relative grid md:grid-cols-2 gap-4 md:gap-6 max-w-5xl mx-auto animate-swipe-left-strong" style={{ animationDelay: '0.1s' }}>
        {items.map((item, idx) => (
          <div
            key={item.number}
            className={`bg-primary rounded-2xl p-6 md:p-8 text-primary-foreground relative overflow-hidden group hover:scale-[1.02] transition-transform duration-300 animate-pop-glow ${idx % 2 === 0 ? 'animate-swipe-left-strong' : 'animate-swipe-right-strong'}`}
